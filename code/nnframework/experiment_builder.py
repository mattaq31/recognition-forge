"""
Contains an experiment builder for normal classification.  Takes care of validation/training runs, saving, loading etc.
...
from nnframework.storage_utils import save_to_stats_pkl_file, load_from_stats_pkl_file, \
    save_statistics, load_statistics


class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
                 test_data, weight_decay_coefficient, use_gpu, continue_from_epoch=-1, save_all_models=False,suppress_logs=True,cleanfiles = False):
        """
        ...
        :param save_all_models: Whether to save all models, or only the best. Default is False.
        ...
        """
        # TODO: implement flags to turn on/turn off saving models completely
        ...
        if test_data:# in case cross validation is used
        ...
        self.experiment_folder = os.path.abspath(os.path.join(const.DIR_EXPERIMENTS, experiment_name))
        ...
        if not os.path.exists(self.experiment_logs):  # If experiment log directory does not exist
            os.makedirs(self.experiment_logs)  # create the experiment log directory
        if not os.path.exists(self.experiment_saved_models):  # If experiment saved models directory does not exist
    ...
    def save_model(self, model_save_dir, model_save_name, model_idx, best_validation_model_idx,
        ...
        file_name = model_save_name if not self.save_all_models else "{}_{}".format(model_save_name, str(model_idx))
        torch.save(state, f=os.path.join(model_save_dir, file_name))  # save state at prespecified filepath
    ...
    def load_model(self, model_save_dir, model_save_name, model_idx):
        ...
        """
        file_name = model_save_name if not self.save_all_models else "{}_{}".format(model_save_name, str(model_idx))
        state = torch.load(f=os.path.join(model_save_dir, file_name))
    ...
    def run_experiment(self):
            ...
            if val_mean_accuracy > self.best_val_model_acc:  # if current epoch's mean val acc is greater than the saved best val acc then
                ...
                if not self.save_all_models:
                    ...
                    self.save_model(model_save_dir=self.experiment_saved_models,
                                    ...
                                    model_save_name="best_model", model_idx=epoch_idx,
                                    best_validation_model_idx=self.best_val_model_idx,
                                    best_validation_model_acc=self.best_val_model_acc)

                    save_statistics(experiment_log_dir=self.experiment_logs, filename='summary.csv',
                                    stats_dict=total_losses, current_epoch=i,
                                    continue_from_mode= False)  # save statistics to stats file.
        ...
        print("Using best model: Epoch {0}, acc: {1:.4f}".format(self.best_val_model_idx, self.best_val_model_acc))
        ...
        if self.test_data:
            if self.suppress_logs:
                ...
                with tqdm.tqdm(total=self.test_data.num_batches) as pbar_test:  # init a progress bar
                    for x, y in self.test_data:  # sample batch
                        loss, accuracy = self.run_evaluation_iter(x=x,
                                                                  y=y)  # compute loss and accuracy by running an evaluation step
                        current_epoch_losses["test_loss"].append(loss)  # save test loss
            ...
            print("Test results: ", " ".join(
                ["{}: {:.4f}".format(key, np.mean(value)) for key, value in test_losses.items()]))
        else:
            test_losses = False
        if self.cleanfiles:
            self.file_cleaner()
        return total_losses, test_losses, self.best_val_model_acc
