...
from skipthoughts_dir.decoding import homogeneous_data

from theano.sandbox.rng_mrg import MRG_RandomStreams as RandomStreams
from collections import defaultdict

from skipthoughts_dir.decoding.decoder_utils import *
from skipthoughts_dir.decoding.layers import get_layer, param_init_fflayer, fflayer, param_init_gru, gru_layer
from skipthoughts_dir.decoding.optim import adam
from skipthoughts_dir.decoding.model import init_params, build_model, build_sampler
from skipthoughts_dir.decoding.vocab import load_dictionary
...
def trainer(X, C, stmodel,
            ...
            optimizer='adam',
            batch_size=16,
            saveto='skipthoughts_dir/aux_data/decoder_toy',
            dictionary='skipthoughts_dir/aux_data/dummy.dict',
            embeddings=None,
            saveFreq=100,
            sampleFreq=50,
            reload_=False):
    ...
    uidx = 0
    lrate = 0.01
    for eidx in tqdm(range(max_epochs)):
        n_samples = 0
        ...
        for x, c in train_iter:
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                numpy.savez('%s_epoch%d.npz'%(saveto, eidx), history_errs=[], **params)
                pkl.dump(model_options, open('%s_epoch%d.pkl'%(saveto, eidx), 'wb'))
                print('Done')
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                for jj in range(numpy.minimum(10, len(ctx_s))):
                    ...
                    #print('Truth ',jj,': ',)
                    truth_output, sample_output = '', ''
                    for vv in x_s[:,jj]:
                        if vv == 0:
                            break
                        if vv in word_idict:
                            truth_output += ' ' + (word_idict[vv])
                        else:
                            truth_output += ' ' + 'UNK'
                    #print(truth_output)
                    #print('')
